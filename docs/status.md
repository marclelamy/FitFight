...
| Accept / Join | Invites still accept in the fight. Joinable fights open the same Accept/Join screen from a code, the live list, or a shared link. A shared `staging.fitfight.app/j/CODE` page offers TestFlight when the app is missing; after install, Open in FitFight or the original link continues the join. Production uses the App Store when that listing URL is set. Joins go through the server. Leave a joinable or repeating fight from the fight itself so the next window does not copy you in. |
