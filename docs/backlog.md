...
- **Collect from vendor APIs directly; Apple Health only for Watch/iPhone.** Inventory is [`research/data-sources.md`](research/data-sources.md). People will not turn on Apple Health export, so WHOOP, Oura, Polar, Garmin, COROS, and Strava must be connected in FitFight. Still private, not Fight scoring. First adapters: WHOOP and Oura (workouts plus Recovery/Readiness), then HealthKit workouts for Watch/iPhone, then Polar. Garmin and COROS wait on Marc’s developer applications. **Strava is in:** connect it and collect. Marc emails `developers@strava.com` with the private-Fight UX before Strava numbers appear on someone else’s card. Amazfit, Nike Run Club, and Peloton have no public API — HealthKit fallback only. Skip Fitbit/Pixel/Google this wave.
...
- **Restore bounded full-fidelity HealthKit ingestion only when a shipped feature needs it.** The Steps MVP still keeps only Apple's merged exact Fight-window totals and the relevant merged daily chart buckets. The Next collection work is vendor APIs plus a bounded HealthKit workout slice for Apple Watch/iPhone — not a license to restore raw samples, deletion anchors, per-source statistics, device/source metadata, or another archive transport. Those still need explicit Collection consent, bounded backfill, retention and deletion policy, edit/deletion reconciliation, provenance shown to Users, and operational size limits. Never calculate Steps by naïvely summing overlapping raw samples or source totals; Apple's merged aggregate remains the reference unless a reviewed Metric specification replaces it.
