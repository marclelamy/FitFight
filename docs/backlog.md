...
- **Replay and recurring fights.** Rolling recurring windows and join codes already shipped. Still Later: a Redo / Replay button, win/loss history across windows, and workplace / campus extras. Workplace (Station F) is the same Series object plus a join code, not a new HR product — proposal in [`research/workplace-recurring-fights.md`](research/workplace-recurring-fights.md). No extra screens yet; do not invent one. Do not implement that proposal’s remaining forks until Marc answers them and moves this up.
