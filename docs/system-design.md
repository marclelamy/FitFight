...
- **Staging**: GitHub branch `develop` for the hosted database and site. TestFlight binaries are cut from GitHub `staging`, then `preview`. A long-lived persistent branch of the production Supabase project, also named `develop`, with fake/test Users, stable branch credentials, staging OAuth callbacks, and staging secrets.
