...
A cloud agent writes SQL in `supabase/migrations` and tests in `supabase/tests`, then opens a PR **into `develop` only when Marc explicitly asks for a PR**. Marc merges that. The persistent Supabase branch `develop` picks it up. TestFlight is a later merge to `staging` or `preview`. Production only changes when Marc merges `preview` → `main`. Agents do not get the database password or `sb_secret_...` key, and they do not merge unless Marc asked.
...
| `staging` | TestFlight cut | Same persistent `develop` project |
| `preview` | Next TestFlight cut | Same persistent `develop` project |
| `main` | Production | The main project |

In **Branching**, create one long-lived branch named **`develop`** (not `staging`). It tracks the GitHub `develop` branch. Feature PRs merge into `develop`. GitHub `staging` and `preview` only cut TestFlight binaries; they must not become extra hosted databases. When Marc wants a TestFlight, he merges `develop` → `staging`, then `staging` → `preview`. When he wants production, he merges `preview` into `main`.
...
Marc’s extra lock (GitHub ruleset **Protect main**): target **`main`, `develop`, `staging`, and `preview`** → require a pull request, required approvals **0**, require status check **Migrations and RLS**, block force pushes. That stops a push onto those branches without the database check. You still tap merge. You cannot approve your own PR, which is why approvals stay at 0. If the ruleset still lists only `main` and `develop`, add `staging` and `preview`.
...
Every TestFlight binary talks to the develop project. The workflow runs on push to `staging` or `preview`, or manual `workflow_dispatch` on those branches, and rejects `main` and `develop`. CI writes `FitFight/Generated/BuildEnv.swift` before archive. GitHub variables `SUPABASE_STAGING_*` override if set; otherwise the known develop URL and publishable key are compiled in. Production configuration belongs only to the future App Store flow. See [`shipping.md`](shipping.md). What is live on the phone: [`status.md`](status.md).
